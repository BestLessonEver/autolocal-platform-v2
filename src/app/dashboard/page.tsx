...
function getSiteUrl(data: { custom_domain: string | null; website_current: string | null; slug: string }): string {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="absolute right-0 top-0 bottom-0 w-72 bg-[#18181b] border-l border-white/10 p-6 space-y-5 animate-slide-left" onClick={e => e.stopPropagation()}>
        ...
        {/* Actions */}
        <a href={getSiteUrl(data)} target="_blank" className="block w-full py-3 rounded-xl bg-indigo-600 text-white text-sm font-bold text-center hover:bg-indigo-500 transition">{getSiteLabel(data)}</a>
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white">
      ...
      <header className="sticky top-0 z-50 bg-[#09090b]/95 backdrop-blur border-b border-white/10 px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-3 flex-1">
            ...
            <a href={getSiteUrl(data)} target="_blank" className="shrink-0 px-4 py-1.5 rounded-lg bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-500 transition">{getSiteLabel(data)}</a>
      ...
      <GoLiveFlow
        ...
        email={userEmail || data.email || data.contact_email || ''}
          ...
          <div className="hidden lg:block w-[420px] shrink-0">
            <div className="sticky top-20">
              <div className="flex items-center justify-between mb-2">
                ...
                <a href={getSiteUrl(data)} target="_blank" className="text-xs text-indigo-400 hover:underline">{getSiteLabel(data).replace(' ↗','') || 'Full size'} ↗</a>
              </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#09090b]/95 backdrop-blur border-t border-white/10 px-4 py-3 flex items-center gap-3">
        ...
        <a href={getSiteUrl(data)} target="_blank" className="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold text-center hover:bg-indigo-500 transition">
          {getSiteLabel(data)}
        </a>
      </div>
      ...
      <section id="sec-seo-tips" className="px-4 py-6 space-y-4">
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { icon: '📝', title: 'Write a detailed About section', desc: 'Search engines and AI assistants use your description to understand your business. Be specific — mention your city, services, and what makes you different.', field: 'about' },
              { icon: '🏷️', title: 'List all your services with descriptions', desc: 'Each service becomes structured data that Google and ChatGPT can reference. Include prices when possible.', field: 'services' },
